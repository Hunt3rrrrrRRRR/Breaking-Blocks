...
import { AppState, VoxelData, SavedModel, Palette } from './types';
import { DEFAULT_PALETTE } from './utils/voxelConstants';
...
const App: React.FC = () => {
  ...
  // --- State for Custom Palette ---
  const [palette, setPalette] = useState<Palette>(() => {
    const saved = localStorage.getItem('voxel_palette');
    return saved ? JSON.parse(saved) : DEFAULT_PALETTE;
  });

  useEffect(() => {
    localStorage.setItem('voxel_palette', JSON.stringify(palette));
  }, [palette]);
  ...
  const handlePromptSubmit = async (prompt: string) => {
    ...
    try {
        ...
        // Construct detailed color guidance
        const paletteStr = palette.join(', ');
        const colorGuidance = `
            COLOR PALETTE: [${paletteStr}].
            MANDATORY RULES FOR COLOR:
            1. You MUST use colors ONLY from the provided PALETTE.
            2. If you need a color that isn't exactly in the palette, choose the closest visually matching one from the list.
            3. Do NOT invent new hex codes.
        `;
        ...
        if (promptMode === 'morph' && engineRef.current) {
            ...
            systemContext = `
                CONTEXT: You are creating a brand new voxel art scene.
                ${colorGuidance}
            `;
        }
                ...
                responseSchema: {
                    ...
                    items: {
                        ...
                        properties: {
                            ...
                            y: { type: Type.INTEGER },
                            z: { type: Type.INTEGER },
                            color: { type: Type.STRING, description: "Hex color code from the provided palette" }
                        },
  ...
  return (
    <div className="relative w-full h-screen bg-[#f0f2f5] overflow-hidden">
      ...
      <PaletteManager 
        ...
        onResetPalette={() => setPalette(DEFAULT_PALETTE)}
      />
